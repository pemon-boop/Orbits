...
def LambertIzzo(mu : float, pos1 : np.ndarray, pos2 : np.array, tof : float , nrev : int = 0, solverTol : float = 1e-14):
    ...
    if bad:
        ...
        vel1, vel2, exitflag = LambertLancasterBlanchard(mu, r1vec.scalarMultiply(r1), r2vec.scalarMultiply(r1), longway*tf*T, leftbranch*m, solverTol)
    ...
    if (x < 1): # ellipse
        ...
        beta = longway * 2*np.asinh(np.sqrt((c-s)/2/a))
        alfa = 2*np.arccosh(x)
        psi  = (alfa-beta)/2
